...
# Thank DerVVulfman for the following, quite evil suggestions:

# Your players might now face a whole new set of foes by using a very specific
# script call. Feel free to change the encounter list as many times as needed.

# You can set a custom "Force Battle" terrain tag for specific maps.
# Just scroll down to the FightAlways module and edit the TERRAIN_TAG hash
# and its default and custom values accordingly.
...
module ChooseFoes
  ...
  module FightAlways
    TERRAIN_TAG = {} # Better leave this line alone!
    TERRAIN_TAG.default = 1 #<= Feel free to change it.
    # Custom Terrain Tags #
    TERRAIN_TAG[1] = 7
  end
...
class Scene_Map
  include ChooseFoes::FightAlways
  alias :kyon_chs_foes_scn_mp_cll_btl :call_battle
  def call_battle
    map_id = $game_map.map_id
    terrain_tag = $game_map.terrain_tag(*$game_player.xy)
    list = $game_map.encounter_list
    if TERRAIN_TAG[map_id] == terrain_tag or list.size < 2
